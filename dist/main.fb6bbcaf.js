...
var notesNatural = ['A', 'B', 'C', 'D', 'E', 'F', 'G'];
var notesAccidentals = ['A♯', 'B♭', 'C♯', 'D♯', 'E♭', 'F♯', 'G'];
var notesEnharmonic = ['A♭', 'C♭', 'D♭', 'F♭', 'G'];
var accidentalsCheckbox = document.querySelector('#accidentals');
var enharmonicCheckbox = document.querySelector('#enharmonic');
var noteBtn = document.querySelector('button');
var noteText = document.querySelector('.chosen-note');

function generateRandomNote() {
  var currentNote = noteText.innerText; // Compile our notes based on checkboxes

  var notes = [].concat(notesNatural);

  if (accidentalsCheckbox.checked) {
    notes.push.apply(notes, notesAccidentals);
  }

  if (enharmonicCheckbox.checked) {
    notes.push.apply(notes, notesEnharmonic);
  } // Generate a random number between 0 and total number of notes


  var randomNote = notes[Math.floor(Math.random() * Math.floor(notes.length))];
  ;
